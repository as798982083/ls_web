...
$.extend({
    ...
    jsPlugs_ProgressBar: function (e) {
        ...
        $.win().$("#dvJsPlugsprogressbarDialog").dialog({
            ...
            open: function () {
                ...
                tmprogressbar = window.setInterval("setProgressBarValue()", 300);
            }
        });
    ...
    jsPlugs_Panel: function (e) {
        ...
        $.win().$("body").append("<div id='dvJsPlugsPanel' title='" + t + "' style='width:" + (w + 120) + "px !important;'><iframe src='" + e.url + d + "' style='margin:0px auto;height:" + (h - 70) + "px;width:" + (w - 20) + "px' frameborder='0' scrolling='0'></iframe></div>");
...
$.fn.extend({
    ...
    jsPlugs_pageRecord: function (e) {
        this.each(function () {
            ...
            e.pageSize = e.pageSize || 12;
            e.pageNumWidth = e.pageNumWidth || 200, 	//翻页  页码的宽度，可选   默认为 300
			e.firstText = e.firstText || "<i class='icon-double-angle-left'></i>", 	//first的文字  可选   默认为 First
			e.prevText = e.prevText || "<i class='icon-angle-left'></i>", 	//上一页的文字  可选   默认为 Prev
			e.nextText = e.nextText || "<i class='icon-angle-right'></i>", 	//下一页的文字  可选   默认为Next
			e.lastText = e.lastText || "<i class='icon-double-angle-right'></i>", 		//尾页的文字    可选  默认为   Last
            ...
            $.ajax({
                ...
                success: function (d) {
                    ...
                    if (pageCount > 0) {
                        ...
                        plugs_pages += "<div id='dvpagerecordNumGo'><input type='text' id='pageRecordtxtNumGo' /><input type='button' id='pageRecordBtnNumGo' class='btn btn-primary'  onclick='pageRecordBtnNumGo_click()' value='" + e.btnGoText + "' /><div></div>";
                        ...
                        /*设定选中   效果*/
                        $("#dvpagerecordcontent li").eq(e.currentPage - 1).addClass("btn btn-primary").css("color", "#fff").css("border", "1px solid #cccccc").css("height", 21).css("padding", "6px 13px");
                        //            			/*设定选中的位置*/
                        ...
                        /*  设定宽度   设定表格  效果  */
                        if (pageCount < 5) e.pageNumWidth = 38 * pageCount;
                        $("#dvpagerecordcontent").css("width", e.pageNumWidth - 3);
                        /*分页ul li 移入  移出	  特效*/
                ...
                complete: function (request, settings) {
                    ...
                    if (e.hasEditColumn && e.hasEditColumn.clickEvent) {
                        for (var i = 0; i < $(".td_column_edit").length; i++) {
                            ...
                            $(".td_column_edit").eq(i).children(0).bind("click", { key: id }, e.hasEditColumn.clickEvent);
                        }
                    }
                    if (e.hasDeleteColumn && e.hasDeleteColumn.clickEvent) {
                        for (var i = 0; i < $(".td_column_delete").length; i++) {
                            ...
                            id = id.split('_')[id.split('_').length - 1];
                            $(".td_column_delete").eq(i).children(0).bind("click", { key: id }, e.hasDeleteColumn.clickEvent);
                        }
                    }
